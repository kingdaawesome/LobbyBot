...
try:
    ...
    print(Fore.RED + f'[FORTNITEPY] [N/A] [ERROR] Failed to import 1 or more modules, run "Setup.bat".')
    exit()

print(f'  ')
print(color.RED + f'Straybot 1.0')
print(color.RED + f'  -Starting Up')
print(f'  ')
...
with open('config.json') as f:
    print(f' [STRAYBOT] [{getTime()}] Loading config.')
    data = json.load(f)
    print(f' [STRAYBOT] [{getTime()}] Config loaded.')
    
debug = 'False'
if debug == 'True':
    print(f' [STRAYBOT] [{getTime()}] Debug logging is on.')
    debugOn()
else:
    print(f' [STRAYBOT] [{getTime()}] Debug logging is off.')
...
async def event_ready():
    print(Fore.GREEN + ' [STRAYBOT] [' + getTime() + '] Client ready as {0.user.display_name}.'.format(client))
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        if invite.sender.display_name not in data['BlockList']:
            try:
                await invite.accept()
                print(Fore.GREEN + f' [STRAYBOT] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
            except Exception as e:
                pass
        elif invite.sender.display_name in data['BlockList']:
            print(Fore.GREEN + f' [STRAYBOT] [{getTime()}] Never accepted party invite from' + Fore.RED + f' {invite.sender.display_name}')
    if data['joinoninvite'].lower() == 'false':
        if invite.sender.display_name in data['FullAccess']:
            await invite.accept()
            print(Fore.GREEN + f' [STRAYBOT] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
        else:
            print(Fore.GREEN + f' [STRAYBOT] [{getTime()}] Never accepted party invite from {invite.sender.display_name}')
...
async def event_friend_request(request):
    if data['friendaccept'].lower() == 'true':
        if request.display_name not in data['BlockList']:
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] Accepted friend request from: {request.display_name}")
            except Exception as e:
                pass
        elif request.display_name in data['BlockList']:
            print(f" [STRAYBOT] [{getTime()}] Never Accepted friend reqest from: " + Fore.RED + f"{request.display_name}")
    if data['friendaccept'].lower() == 'false':
        if request.display_name in data['FullAccess']:
            ...
            print(f" [STRAYBOT] [{getTime()}] Never accepted friend request from: {request.display_name}")

@client.event
async def event_party_member_join(member):
    if client.user.display_name != member.display_name:
        print(f" [STRAYBOT] [{getTime()}] {member.display_name} has joined the lobby.")
...
async def event_friend_message(message):
    ...
    print(' [STRAYBOT] [' + getTime() + '] {0.author.display_name}: {0.content}'.format(message))
    if "!playlist" in args[0].lower():
        try:
    ...
    if "!cid" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] CID for {cosmetic.name}: {cosmetic.id}")
            except BenBotAsync.exceptions.NotFound:
    ...
    if "!bid" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] BID for {cosmetic.name}: {cosmetic.id}")
            except BenBotAsync.exceptions.NotFound:
    ...
    if "!eid" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] EID for {cosmetic.name}: {cosmetic.id}")
            except BenBotAsync.exceptions.NotFound:
    ...
    if "!pid" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] PID for {cosmetic.name}: {cosmetic.id}")
            except BenBotAsync.exceptions.NotFound:
    ...
    if "!purpleskull" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] Client's Skin set to Purple Skull Trooper")
            except Exception as e:
                pass
    ...
    if "!pinkghoul" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] Client's Skin set to Pink Ghoul Trooper")
            except Exception as e:
                pass
    ...
    if "!brainiacghoul" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [STRAYBOT] [{getTime()}] Client's Skin set to Brainiac Ghoul Trooper")
            except Exception as e:
                pass
    ...
    if "!purpleportal" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [STRAYBOT] [{getTime()}] Client's Backpack set to Purple Ghost Portal")
    ...
    if "!banner" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [STRAYBOT] [{getTime()}] Banner set to; {args[1]} {args[2]} {args[3]}")
    ...
    if "CID_" in args[0]:
        if message.author.display_name in data['BlockList']:
            ...
            print(f' [STRAYBOT] [{getTime()}] Skin set to ' + args[0])
    ...
    if "!variants" in args[0]:
        if message.author.display_name in data['BlockList']:
            ...
            print(f' [STRAYBOT] [{getTime()}] Set variants of {args[1]} to {args[2]} {args[3]}.')
    ...
    if "!checkeredrenegade" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [STRAYBOT] [{getTime()}] Client's Skin set to Checkered Renegade")
    ...
    if "!mintyelf" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [STRAYBOT] [{getTime()}] Client's Skin set to Minty Elf")
    ...
    if "!echo" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(f' [STRAYBOT] [{getTime()}] ' + color.GREEN + 'Sent Message:' + color.END + f' {joinedArguments}')
        else:
    ...
    if "!admin" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            if len(args) == 1:
                ...
                print(f' [STRAYBOT] [{getTime()}] Please specify if you want to add or remove a user from the admin list, using ' + color.GREEN + '!admin add ' + color.END + 'or ' + color.GREEN + '!admin remove' + color.END)
            if len(args) == 2:
                if args[1].lower() == 'add':
                    ...
                    if content == "yes":
                        ...
                        with open('config.json', 'w') as f:
                            ...
                            print(f" [STRAYBOT] [{getTime()}] Removed " + color.GREEN + f"{user.display_name}" + color.END + " as an admin")
                ...
                try:
                    if args[1].lower() == 'add':
                        if user.display_name not in data['FullAccess']:
                            ...
                            with open('config.json', 'w') as f:
                                json.dump(data, f, indent=4)
                                print(f" [STRAYBOT] [{getTime()}] Added " + color.GREEN + f"{user.display_name}" + color.END + " as an admin")
                                await message.reply(f"Added {user.display_name} as an admin.")
                        elif user.display_name in data['FullAccess']:               
                            print(f" [STRAYBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is already an admin")
                        ...
                        if user.display_name in data['FullAccess']:
                            ...
                            print(f" [STRAYBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is not an admin")
                            await message.reply(f"{user.display_name} is not an admin.")
                except AttributeError:
                    pass
                    print(f" [STRAYBOT] [{getTime()}] Can't find user: " + color.GREEN + f"{joinedArgumentsAdmin}" + color.END)
        ...
        if message.author.display_name in data['FullAccess']:
            ...
            if len(args) == 2:
                if args[1].lower() == 'add' or args[1].lower() == 'remove':
                    ...
                    print(f' [STRAYBOT] [{getTime()}] Please specify the name of the user you want to add/remove from the block list')
                else:
                    await message.reply('Invalid usage, try !blocklist add <username> or !blocklist remove <username>')
                    print(f' [STRAYBOT] [{getTime()}] Invalid usage, try ' + color.GREEN + '!BlockList add <username> ' + color.END + 'or ' + color.GREEN + '!BlockList remove <username>' + color.END)
            if len(args) >= 3:
                joinedArgumentsAdmin = " ".join(args[2:])
                user = await client.fetch_profile(joinedArgumentsAdmin)
                if args[1].lower() == 'add':
                    if user.display_name not in data['FullAccess'] and user.display_name not in data['BlockList']:
                        ...
                        with open('config.json', 'w') as f:
                            ...
                            print(f" [STRAYBOT] [{getTime()}] Added " + color.GREEN + f"{user.display_name}" + color.END + " to the blocked list.")
                    elif user.display_name in data['FullAccess']:
                        await message.reply(f"{user.display_name} can not be added to the blocked list.")
                        print(f" [STRAYBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " cannot be added to the blocked list.")
                    elif user.display_name in data['BlockList']:               
                        await message.reply(f"{user.display_name} is already on the blocked list.")
                        print(f" [STRAYBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is already on the blocked list.")
                elif args[1].lower() == 'remove':
                    if user.display_name in data['BlockList']:
                        data['BlockList'].remove(user.display_name)
                        with open('config.json', 'w') as f:
                            json.dump(data, f, indent=4)
                            print(f" [STRAYBOT] [{getTime()}] Removed " + color.GREEN + f"{user.display_name}" + color.END + " from the blocked list.")
                    elif user.display_name not in data['BlockList']:
                        print(f" [STRAYBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is not on the blocked list.")
        if message.author.display_name not in data['FullAccess']:
            if len(args) >= 3 and args[1].lower() == 'add':
                await message.reply(f"Password?")
                res = await client.wait_for('friend_message')
    ...
    if "!status" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(f' [STRAYBOT] [{getTime()}] Status set to {joinedArguments}.')
        else:
            if message.author.display_name not in data['FullAccess']:
                await message.reply(f"You don't have access to this command!")
    ...
    if "!leave" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(Fore.GREEN + f' [STRAYBOT] [{getTime()}] Left the party as I was requested.')
        else:
            if message.author.display_name not in data['FullAccess']:
                await message.reply(f"You don't have access to this command!")
    ...
    if "!kick" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if member is None:
            ...
            try:
                ...
                print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] Kicked user: {member.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Couldn't kick {member.display_name}, as I'm not party leader.")
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Failed to kick member as I don't have the required permissions." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"You don't have access to this command!")
    ...
    if "!join" in args[0] and message.author.display_name in data['FullAccess']:
        ...
        if friend is None:
            ...
            print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Unable to join user: {joinedArguments}, are you sure the bot has them added?" + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"You don't have access to this command!")
        else:
            try:
    ...
    if "!invite" in args[0].lower():
        ...
        if friend is None:
            await message.reply(f"Unable to invite that user, are you sure the bot has them added?")
            print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Unable to invite user: {joinedArguments}, are you sure the bot has them added?" + Fore.WHITE)
        else:
            try:
                await friend.invite()
                await message.reply(f"Invited user: {friend.display_name}.")
                print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] Invited user: {friend.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Something went wrong trying to invite {friend.display_name}")
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Something went wrong while trying to invite {friend.display_name}" + Fore.WHITE)           
    ...
    if "!add" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if user is None:
            ...
            print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Unable to find a player with the name {joinedArguments}")
        else:
            try:
                if (user.id in friends):
                    await message.reply(f"I already have {user.display_name} as a friend.")
                    print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] You already have {user.display_name} added as a friend.")
                else: 
                    await client.add_friend(user.id)
                    await message.reply(f"Sent a friend request to {user.display_name}")
                    print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] {client.user.display_name} sent a friend request to {user.display_name}" + Fore.WHITE)
            except Exception as e:
                pass
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Something went wrong adding {joinedArguments}" + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"You don't have access to this command!")
        ...
        if user is None:
            ...
            try:
                if (user.id in friends):
                    ...
                    print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] {client.user.display_name} removed {user.display_name} as a friend.")
                else: 
                    await message.reply(f"I don't have {user.display_name} as a friend.")
                    print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] {client.user.display_name} tried removing {user.display_name} as a friend, but the client doesn't have the friend added." + Fore.WHITE)
            except Exception as e:
                pass
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Something went wrong removing {joinedArguments} as a friend." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"You don't have access to this command!")
    ...
    if "!showfriends" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        try:
            ...
            print(f" [STRAYBOT] [{getTime()}] " + Fore.WHITE + "Friends List: " + Fore.GREEN + f"{len(onlineFriends)} Online " + Fore.WHITE + "/" + Fore.LIGHTBLACK_EX + f" {len(offlineFriends)} Offline " + Fore.WHITE + "/" + Fore.LIGHTWHITE_EX + f" {len(onlineFriends) + len(offlineFriends)} Total")
    ...
    if "!members" in args[0].lower() and message.author.display_name in data['FullAccess']:
            ...
            print(f" [STRAYBOT] [{getTime()}] " + Fore.WHITE + "There are " + Fore.LIGHTWHITE_EX + f"{len(partyMembers)} members in client's party:")
    ...
    if "!promote" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if member is None:
            ...
            try:
                ...
                print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] Promoted user: {member.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Couldn't promote {member.display_name}, as I'm not party leader.")
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Failed to promote member as I don't have the required permissions." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"You don't have access to this command!")
    ...
    if "Playlist_" in args[0]:
        try:
            ...
            print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] Failed to set gamemode as I don't have the required permissions." + Fore.WHITE)
    ...
    if args[0] == "!id":
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(Fore.GREEN + f" [STRAYBOT] [{getTime()}] {joinedArguments}'s Epic ID is: {user.id}")
            except AttributeError:
                await message.reply(f"I couldn't find an Epic account with the name: {joinedArguments}.")
                print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] I couldn't find an Epic account with the name: {joinedArguments}.")

try:
    client.run()
except fortnitepy.AuthException as e:
    print(Fore.RED + f" [STRAYBOT] [{getTime()}] [ERROR] {e}")
